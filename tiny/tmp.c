...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    // listenfd: 연결 요청을 수신하는 소켓의 파일 디스크립터입니다. 일반적으로 socket() 함수를 호출하여 생성한 소켓 파일 디스크립터를 전달합니다.
    // (SA *)&clientaddr: 클라이언트의 주소 정보를 저장할 구조체 변수 clientaddr의 주소값을 struct sockaddr * 형태로 캐스팅하여 전달합니다. Accept() 함수는 클라이언트의 주소 정보를 clientaddr 구조체에 저장합니다.
    // Accept() : listenfd에 대한 클라이언트의 연결 요청이 들어오면 해당 요청을 수락하고, 클라이언트와 연결된 새로운 소켓 식별자를 반환
    connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);  // line:netp:tiny:accept
    
    /* 연결이 성공했다는 메세지를 위해. Getnameinfo를 호출하면서 hostname과 port가 채워진다. */
    // Getnameinfo() : 주어진 clientaddr 구조체에서 IP 주소와 포트 번호를 추출하여 해당 정보를 이용하여 호스트 
    // 이름과 서비스 이름을 검색하고, 검색 결과를 hostname과 port 변수에 저장하는 함수
    Getnameinfo((SA *)&clientaddr, clientlen, hostname, MAXLINE, port, MAXLINE, 0);
    // Accepted connection from (143.248.220.176, 49541)
...
void doit(int fd)
{
  ...
  Rio_readlineb(&rio, buf, MAXLINE); // RIO 버퍼에서 한 줄의 데이터를 읽어와서 buf에 저장하는 역할
